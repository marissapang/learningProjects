...
public abstract class Graph {
	...
	public List<Integer> degreeSequence() {
		...
		//1. loop through each vertices, and count all of the neighbors
		//2. sort list to put in sequential order
		
		List<Integer> seq = new ArrayList<Integer>();
		for (int i = 0; i < getNumVertices(); i++) {
			int deg = this.getInNeighbors(i).size() + this.getNeighbors(i).size();
			seq.add(deg);
		}
	...
	public static void main (String[] args) {
		...
		// GraphLoader.loadRoadMap("data/maps/claremont.map", graphFromFile);
