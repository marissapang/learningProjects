...
public abstract class Document {
	...
	protected int countSyllables(String word)
	{
		...
		for (char c: "aiouyAIOUY".toCharArray()) {
			vowelArray.add(c);
		}
		
		for (int i = 0; i < word.length(); i++) {
			if (vowelArray.contains(word.charAt(i))) {
				if (prevIsVowel == false) {
					syllableCount += 1;
					prevIsVowel = true;
				} else { 
					// if previous thing is also vowel we don't increment 
				}
			} else if (word.charAt(i) == 'e' || word.charAt(i) == 'E') {
				if (i == word.length()-1 && syllableCount == 0) {
					syllableCount += 1;
				} else if (i != word.length() - 1 && prevIsVowel == false) {
					syllableCount += 1;
					prevIsVowel = true;
				}
			} else { // if vowelArray does not contain aeiou
				prevIsVowel = false;
			}
		}
	...
	{	
		double numWords = new Double(this.getNumWords());
		// System.out.println("numwords is: " + numWords);
		double numSentences = new Double(getNumSentences());
		// System.out.println("Sentences is: " + numSentences);
		double numSyllables = new Double(getNumSyllables());
		// System.out.println("Syllables is: " + numSyllables);
