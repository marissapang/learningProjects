...
class GameModel : NSObject { // base class for most objective C objects
    let dimension : Int
    let threshold : Int
    ...
    func insertTileAtRandomLocation(withValue value: Int) {
        ...
        if openSpots.isEmpty {
            // Don't do anythign if there is no open spots
            return
        }
    ...
    func userHasLost() -> Bool {
        guard gameboardEmptySpots().isEmpty else {
            // Player can't lose before filling up the empty slots
            return false
        }
    ...
    func performMove(direction: MoveDirection) -> Bool {
        // Prepare the generator closure. This closure differs in behavior depending on the direction of the move
        // It is used by the method to generate a list of tiles which should be modified.
        // Depending on the direction this list may represent a single row or a single column, in either direction
    ...
    func condense (_ group: [TileObject]) -> [ActionToken] {
        ...
        for (idx, tile) in group.enumerated() {
            // Go through all the tiles in 'group'. When we see a tile "out of place", create a corresponding Action Token
            switch tile {
            ...
            default: // case when the tile is .empty
                break
            }
        }
        return tokenBuffer;
    ...
    class func quiescentTileStillQuiescent(inputPosition: Int, outputLength: Int, originalPosition: Int) -> Bool { // this is a type method on the class
        // Return wether or not a "NoAction" token still represents an unmoved tile
    ...
    func collapse(_ group: [ActionToken]) -> [ActionToken] {
        ...
        for (idx, token) in group.enumerated(){
            if skipNext {
                // Prior iteration handled a merge. So ski this iteration
                skipNext = false
                continue
            }
            switch token {
            case .singleCombine: // don't do anything if tile has already been combined
                assert(false, "Cannot have a single combine token in input")
            case .doubleCombine: // don't do anything if tile has already been combined
                assert(false, "Cannot have double combine token in input")
            case let .noAction(s, v) // s is source, v is value for ActionToken object
                ...
                // this time hasn't moved yet, but its value matches that of the next tile this is a single merge
            ...
            case let t where (idx < group.count-1 && t.getValue() == group[idx+1].getValue()):
                ...
                // The last time is *not* eligible for a merge
                let next = group[idx+1]
