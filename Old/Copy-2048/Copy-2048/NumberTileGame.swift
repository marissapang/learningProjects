...
class NumberTileGameViewController : UIViewController, GameModelProtocol {
    ...
    func setupGame() {
        ...
        func yPositionForViewAtPostion(_ order: Int, views: [UIView]) -> CGFloat {
            assert(views.count > 0)
            assert(order >= 0 && order < views.count)
            // let viewHeight = views[order].bounds.size.height
            let totalHeight = CGFloat(views.count - 1)*viewPadding + views.map({
                $0.bounds.size.height }).reduce(verticalViewOffset, {$0 + $1})
        ...
        f.origin.y = yPositionForViewAtPostion(0, views: views)
        scoreView.frame = f
        
        f = gameboard.frame
        f.origin.x = xPositionToCenterView(gameboard)
        f.origin.y = yPositionForViewAtPostion(1, views: views)
        gameboard.frame = f
